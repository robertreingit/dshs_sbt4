...
Im folgenden Abschnitt beschäftigen wir uns mit experimentellen Design. Dazu gehört eine Betrachtung wie beobachtete Effekte überhaupt auf Ursachen zurück geführt werden können und welche Möglichkeiten in Form von verschiedenen Experimentellen Designs zur Verfügung stehen, gewünschte Effekte möglichst effektiv zu identifizieren. D.h. es geht unter anderem darum, wie ein Experiment aus statistischer Sicht so konzipiert werden kann,  um die Wahrscheinlichkeit einen bestimmten Effekt zu beobachten möglichst groß zu machen. Dazu schauen wir uns zunächst ein einfaches Beispiel an. Eine zentrale Größe wird dabei immer wieder der Standardfehler $\sigma_e$ spielen. 

## Zuteilung der Stichprobengrößen in zwei Gruppen

Das folgenden Beispiel ist aus @goos2011 entnommen. Es soll ein Mittelwertsunterschied zwischen zwei Gruppen, Gruppe $1$ und Gruppe $2$, untersucht werden. In Gruppe $1$ haben wir eine Stichprobengröße von $n_1$ und entsprechend eine Stichprobegröße $n_2$ in Gruppe $2$. Wir interessieren uns nun für den Unterschied $D$ zwischen den beiden Gruppen.

$$
D = \bar{X}_1 - \bar{X}_2
$$

Der Unterschied $D$ alleine für sich ist noch relativ wenig informativ, da er mit den Einheiten und vor allem vor dem Hintergrund der *Streuung* der Werte zu betrachten ist. D.h. wir benötigen auch ein Maß für die Unsicherheit von $D$. Dies ist wie bereits mehrmals besprochen der Standardfehler. Wenn wir eine lineares Modell ansetzen würden und Stift-und-Papier zücken würden, dann würde sich zeigen, dass der Standardfehler$\sigma^(D)$ von $D$ die folgende Form hat:

$$
\sigma^2(D) = \frac{\sigma_1^2}{n_1} + \frac{\sigma_2^2}{n_2} 
$$

Sei nun der Unterschied $D$ gegeben. Zum Beispiel wollen wir eine Verbesserung in der Lebensqualität betrachten und dann wäre eine Verbesserung von einer Einheit als relevant zu betrachten. Um nun die Wahrscheinlichkeit zu erhöhen einen relevanten Unterschied $D$ zwischen den beiden Gruppen zu finden, wählen wir nun ein experimentelles Design, dass die Varianz $\sigma^2(D)$ bzw. den Standardfehler $\sigma_e(D)$ von $D$ möglichst klein werden lässt. Umso kleiner der Standardfehler umso geringer sind die Streuungen der Stichprobenverteilung und umso geringer unsere Unsicherheit. 

Seien wir in unseren Ressourcen beschränkt und wir haben ein *Budget*, dass es uns erlaubt nur insgesamt $N = 12$ Personen in unser *Experiment* einzuschließen. Eine weitere Vereinfachung sei, dass die Standardabweichungen in beiden Population gleich sind, also $\sigma^2_1 = \sigma^2_2 = \sigma^2 = 1$. Die zentrale Frage ist nun: **Wie können wird diese $N = 12$ Personen auf die beiden Gruppen verteilen, um den Standardfehler möglichst klein zu halten?** In @fig-ed-basics-sample-01 sind die Varianz $\sigma^2(D)$ und der Standardfehler $\sigma_e(D)$ gegen die möglichen Verteilungen der Probanden abgetragen. 

```{r}
#| fig-cap: "Abhängigkeit des Standardfehlers von $D$ mit der Stichprobenverteilung bei gleicher Varianz"
...
ed_d1 <- tibble(n_1 = 1:11, n_2 = seq(11,1), v_d = var_d(n_1), se_d = sqrt(v_d), eff = min(v_d)/v_d) 
ggplot(ed_d1, aes(n_1, se_d)) +
  geom_line() +
  geom_point() +
  scale_x_continuous("Anzahl der Personen in Gruppe 1", breaks=1:11) +
  scale_y_continuous(expression(sqrt(frac(1,n[1])~+~frac(1,n[2]))))
```

Wir können sehen, dass bei gleicher Stichprobenanzahl in beiden Gruppen der Standardfehler den kleinsten Wert annimmt. D.h. bei gleicher Varianz ist die übliche Herangehensweise, beide Gruppen möglichst gleichgroß zu gestalten sinnvoll, aber nicht zwingend. Schauen wir uns daher als nächsten Fall an, was passiert wenn die Varianzen in den beiden Gruppen nicht mehr gleich sind. Sei zum Beispiel $\sigma^2 = 1$ und $\sigma^2 = 9$. Also ein recht extremer Unterschied zwischen den beiden Gruppen. In @fig-ed-basics-sample-02 sind wieder die verschiedenen Werte $\sigma_e(D)$ gegen die Gruppengröße von Gruppe 1 abgetragen. 

```{r}
#| fig-cap: "Abhängigkeit der Varianz und des Standardfehler von $D$ mit der Stichprobenverteilung bei ungleicher Varianz $\\sigma_1^2 = 1, \\sigma_2^2=9$"
#| label: fig-ed-basics-sample-02

n <- 12
sigma_1 <- 1
sigma_2 <- 9
ed_d2 <- tibble(n_1 = 1:11, n_2 = seq(11,1), v_d = var_d12(n_1, sigma_1, sigma_2), se_d = sqrt(v_d), eff = min(v_d)/v_d)  
ggplot(ed_d2, aes(n_1, se_d)) +
  geom_line() +
  geom_point() +
  scale_x_continuous("Anzahl der Personen in Gruppe 1", breaks=1:11) +
  scale_y_continuous(expression(sqrt(frac(1,n[1])~+~frac(1,n[2]))))
...
Wir können sehen, dass jetzt nicht mehr das Design mit gleich großen Stichproben optimal ist, sondern das Design mit $n_1 = 3$ und $n_2 = 9$. Die ungleiche Varianz in den beiden Gruppen hat dazu geführt, dass der minimale Standardfehler dann eingenommen wird, wenn wir eine größere Stichprobe aus Gruppe $2$ als aus Gruppe $1$ ziehen. Wenn wir diese nicht gewusst hätten und in beiden Gruppen wieder mit $n_i = 6$ gearbeitet hätten, hätten wir somit Effizienz eingebüßt. Intuitiv macht diese ungleiche Verteilung auch Sinn. Da die Varianz in Gruppe $2$ größer ist, ist auch unserer Unsicherheit über den Mittelwert in dieser Gruppe größer. Um diese Unsicherheit auszugleichen erhöhen wir die Stichprobenanzahl im Verhältnis zu Gruppe $1$ um mehr Information über diese Gruppe zu erhalten.
...
Wir können jetzt nicht einfach den gleichen Graphen wie in den obigen Fällen benutzen, da wir unser Zeitbudget beachten müssen. Mit etwas ausprobieren kommen wir zu folgendem Ergebnis.
