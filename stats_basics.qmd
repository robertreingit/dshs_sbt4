...
In diem folgenden Abschnitt, wird die Frage behandelt: "Warum beschäftigen wir uns überhaupt mit der Berechnung einer Statistik wenn wir wissenschaftliche Daten untersuchen?". Bei der Beantwortung dieser Frage fallen einem wahrscheinlich meistens Antworten wie: *"Weil das Ergebnis zufällig sein könnte"* oder *"Weil wir wissen wollen ob ein Unterschied auch signifikant ist"* und ähnliche. Diese Antworten sind im Grunde genommen alle korrekt gehen aber an dem Grundlegenden Problem vorbei. Um dieses Problem zu verstehen, ist es notwendig zunächst ein paar Konzepte einzuführen. Diese Konzepte werden in diesem und dem folgenden Kapitel entwickelt.
...
Zunächst wird eine übersichtliche hypothetische Welt definiert. Die Welt, über die eine Aussage getroffen werden soll, besteht aus nur insgesamt $20$ Personen. In @fig-small-world sind die Bewohner dieser Welt einzeln zu dargestellt. Um keine $20$ Namen zu verwenden, sind die Bewohner durchnummeriert von $1$ bis $20$ mit einem Prefix $P$.
...
Zurück zum *Kleine Welt Beispiel*. Um jetzt für das Experiment eine Stichprobe zu ermitteln, wird genutzt, dass die Population bereits durchnummeriert ist. Nun kann ein Zufallszahlengenerator verwendet werden, der zunächst sechs Zahlen für die Stichprobe bestimmt. Beispielsweise die Zahlen $i = \{`r paste(sort(id_s1), collapse=',')`\}$. Die entsprechenden Personen werden aus der Population anhand der ID ausgewählt. Anschließend teilt wieder ein Zufallszahlengenerator die sechs Personen in die beiden Gruppen auf (siehe @fig-stats-basics-sample-01).
