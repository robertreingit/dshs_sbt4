...
s_e_sim <- round(sd(x_bars), 2)
```

Das Ergebnis der Simulation ergibt einen empirischen Standardfehler von $s_{e|Simulation} = `r s_e_sim`$. Das Ergebnis ist nicht exakt gleich, aber im Rahmen der Stichprobenvariabilität, wird der theoretische Wert tatsächlich sehr gut approximiert. Wenn die Anzahl der Simulationsdurchgängen erhöht werden würde, würde entsprechend der Unterschied immer kleiner werden.
:::

Wie aus dem Hinweis und dem Beispiel ersichtlich wird, gilt ein wurzelförmiger Zusammenhang zwischen dem Standardfehler $\sigma_e$ und der Stichprobengröße $N$. Dieser Zusammenhang gilt nicht nur den Standardfehler des Mittelwerts sondern wird im weiteren Verlauf des Skriptums immer wieder auftreten. Daher ist es sinnvoll sich nochmal den Zusammenhang zwischen dem Bruch $\frac{1}{\sqrt{N}}$ und $N$ zu vergegenwärtigen.
